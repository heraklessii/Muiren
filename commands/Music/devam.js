...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (queue.node.isPaused()) {
      ...
      queue.node.resume();
      const resumed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription('▶️ | Senin için şarkıyı devam ettiriyorum!');

      return interaction.reply({ embeds: [resumed] });
      ...
      const notPaused = new EmbedBuilder()
        .setColor(client.color)
        .setDescription('❌ | Şarkı zaten durdurulmamış.');

      return interaction.reply({ embeds: [notPaused], ephemeral: true });
