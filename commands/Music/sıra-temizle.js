...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('sıra-temizle')
    .setDescription("Mevcut bütün şarkı sırasını temizler."),
  ...
  run: async (client, interaction) => {
    ...
    if (queue.tracks.size < 1) {
      ...
      const embed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription(":x: | Sırada silenecek hiçbir şarkı yok.")
    ...
    if (queue.repeatMode === 2) {
      ...
      const embed = new EmbedBuilder()
        .setColor(client.color)
        .setDescription(":x: | **Sıra tekrar modu** açık olduğu için silemiyorum.")
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder()
        .setColor(client.color)
        .setDescription("☑️ | Şarkı listesi başarıyla temizlendi.")],
      ephemeral: false
    });
